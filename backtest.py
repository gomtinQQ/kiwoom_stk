...
class Strategy_ma5_VolumeAverageMethod(bt.Strategy):
    """
    목적 : 매수시 급등주를 발굴하고, 매도시 3가지 type의 Moving Average을 이용하여,  profit이 가장 좋은 조합을 구한다. 
    ma5 : moving average short term 
    ma20: moving average long term 
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (self.datavolume[0] >=  self.maVolume[0] * self.VolumeMultiple) and (self.maShort[0] < self.dataclose[0]) :
            ...
            if self.maShort[0] > self.dataclose[0]:
                # SELL, SELL, SELL!!! (with all possible default parameters)
                # self.log('SELL CREATE, %.2f' % self.dataclose[0])
        ...
        try:
            ...
            for dealtemp in historytrades:
                ...
                strcsvout = "%s,%s,%s,%s,%s,%.5f\n" % (
                    self.params.code, self.params.maPeriodShort,
                    self.params.maVolPeriod, self.params.VolumeMultiple, "rate", profitrate )
                f_csv.write(strcsvout)
            ...
            self.log('code=%s ma5=%s mvol=%s  mvolmulti=%s profit=%.2f' % (self.params.code, self.params.maPeriodShort,
                    self.params.maVolPeriod, self.params.VolumeMultiple, self.broker.getvalue()), doprint=True)
...
def backtracer_MovingAverage5_Volume(strfilename,listcode):
    lencode = len(listcode)
    countleft = lencode
    ...
    for code in listcode:
        try :
            ...
            cerebro.optstrategy(Strategy_ma5_VolumeAverageMethod, code=code, strfilename=strfilename,
                                maPeriodShort=[4,5,6,7,8,10],
                                MovingAverageMethodShort="SimpleMovingAverage",
                                MovingAverageVolume="SimpleMovingAverage",
                                VolumeMultiple=[27,30,33,36,39,40,45], maVolPeriod=[31,35,37,39,41])
...
if __name__ == '__main__':
    ...
    elif clsvar.backtest_method == 'avg5volume' :
        backtracer_MovingAverage5_Volume(strfilename,listcode )
