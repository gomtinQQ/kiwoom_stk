...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        # 조회('0': 남은 데이터 없음, '1': 남은 데이터 있음) , 즉 조회중임을 나타낸다.
        self.inquiry = 0
    ...
    def on_event_connect(self, return_code):
        """
        통신 연결 상태 변경시 이벤트
        ...
        try:
            if return_code == ReturnCode.OP_ERR_NONE:
                if self.get_login_info("GetServerGubun", True):
                    self.msg += "모의 투자서버에 연결 성공" + "\r\n\r\n"
                else:
                    self.msg += "실 투자서버 연결 성공" + "\r\n\r\n"
            else:
    ...
    def on_receive_tr_data(self, screen_no, request_name, tr_code, record_name, inquiry, unused0, unused1, unused2,
        ...
        :param request_name: string - TR 요청명
        :param tr_code: string
        :param record_name: string
        :param inquiry: string - 조회중임('0': 남은 데이터 없음, '1': 남은 데이터 있음)
        """

        self.log.debug("on_receive_tr_data : screen_no: %s, request_name: %s, tr_code: %s, record_name: %s, inquiry: %s" % (
            screen_no, request_name, tr_code, record_name, inquiry))

        # 주문번호와 주문루프
        # 아래의 코드를 대체한다. ==>  GetCommData
        self.order_no = self.comm_get_data(tr_code, "", request_name, 0, "주문번호")

        try:
            self.order_loop.exit()  # 나중에 다시 고민한다.
        except AttributeError:
            pass
        ...
        if request_name == "예수금상세현황요청":
            estimate_day2_deposit = self.get_comm_data(tr_code, request_name, 0, "d+2추정예수금")
    ...
    def on_receive_msg(self, screen_no, request_name, tr_code, msg):
        """
        수신 메시지 이벤트
    ...
    def on_receive_real_condition(self, code, event, condition_name, condition_index):
        """ OpenAPI ID : 51
        ...
        print("[on_receive_real_condition]")
    ...
    def get_login_info(self, tag, is_connect_state=False):
        ...
        GetServerGubun: 접속서버 구분을 반환합니다.(0: 모의투자, 그외: 실서버) --> 잘못 된 정보
                : 모의 투자 서버:  "1"
                : 실 투자 서버 : "" ( 공백 )
    ...
    def comm_get_data(self, code, real_type, field_name, index, item_name):
        ...
        하지만, 일부 TR에서 사용상 제약이 있음므로 이 함수 대신 GetCommData()함수를 사용하시기 바랍니다.
    ...
    def get_master_listed_stock_cnd(self):
        """ OpenAPI ID: 17
    ...
    def get_master_listed_stock_date(self, strCode):
        """ OpenAPI ID: 19
        종목코드의 상장일을 반환한다.
    ...
    def get_master_last_price(self, strCode):
        """ OpenAPI ID: 20
        종목코드의 전일가를 반환한다
        :param strCode: 종목코드
        :return:
    ...
    def get_master_stock_state(self, strCode):
        """ OpenAPI ID: 21
        종목코드의 종목상태를 반환한다.
    ...
    def get_data_count(self, strRecordName):
        """ OpenAPI ID: 22
        레코드의 반복개수를 반환한다
    ...
    def get_output_value(self, strRecordName, nRepeatIdx,nItemIdx):
        """ OpenAPI ID: 23
        레코드의 반복순서와 아이템의 출력순서에 따라 수신데이터를 반환한다.
        :param strRecordName:
        :param nRepeatIdx: 반복순서
        :param nItemIdx: 아이템 순서
        :return:
        """
        pass

    def get_comm_data(self, trcode, RecordName, index, item_name):
        """ OpenAPI ID: 24
        ...
        :return: 
        """
        ret = self.dynamicCall("GetCommData(QString, QString, int, QString)", trcode, RecordName,
                               index, item_name)
        return ret.strip()
    ...
    def get_theme_group_list(self,nType):
        """ OpenAPI ID: 27
        테마코드와 테마명을 반환한다.
        :param nType: 정렬순서 (0:코드순, 1:테마순)
        :return: 코드와 코드명 리스트
        """
        return self.dynamicCall("GetThemeGroupList(int)",  nType)

    def get_theme_group_code(self, strThemeCode):
        """ OpenAPI ID: 28
        테마코드에 소속된 종목코드를 반환한다
        :param strThemeCode: 테마코드
        :return:
        """
        return self.dynamicCall("GetThemeGroupCode(QString)", strThemeCode)
